...
const useProfileEditLogic = () => {
  ...
  // 处理城市选择
  const handleCitySelect = async (province: string, city: string, locationCode: string) => {
    setCityPickerVisible(false);
    const residence = city || province;

    try {
      setIsSaving(true);
      const response = await profileEditApi.updateResidence(residence, locationCode);

      if (response.code === 200) {
        setProfileData(prev => prev ? { ...prev, residence, locationCode } : null);
      } else {
        Alert.alert('保存失败', response.message || '请稍后重试');
      }
    } catch (err: any) {
